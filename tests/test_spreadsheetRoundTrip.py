...
class TestSpreadSheet(BaseTest):
    ...
    def setUp(self, debug=False, profile=True):
        ...
        self.samples = samples = {"Persons": self.testLoD, "Persons2": self.testLoD}
    ...
    def assertLodEqual(
        self, expectedLOD, actualLOD, checkType: bool = True, errMsg: str = None
    ):
        """
        ...
        for i, record in enumerate(expectedLOD):
            for key, expectedValue in record.items():
                ...
                if checkType:
                    self.assertEqual(
                        type(expectedValue), type(actualLOD[i].get(key)), errMsg
                    )
    ...
    def testRoundTrip(self):
        ...
        for spreadSheetType in spreadSheetTypes:
            ...
            self.assertLodEqual(
                self.testLoD,
                extractedData,
                errMsg=f"when testing {spreadSheetType.__name__}",
            )
    ...
    def testLoading(self):
        ...
        for spreadSheetType in spreadSheetTypes:
            ...
            self.assertIsInstance(
                docReloaded, spreadSheetType, f"when testing {spreadSheetType.__name__}"
            )
            self.assertTrue(
                len(docReloaded.tables) == 2, f"when testing {spreadSheetType.__name__}"
            )
    ...
    def testCSV(self):
        ...
        docReloaded.loadFromFile(
            f"{self.tmpDir.name}/{buffer.name}", samples=self.samples
        )
